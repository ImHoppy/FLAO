...
Options:
    ...
    --backup-all-scripts [path]
                       ...
                       (default: fivem-scripts-backup-<date>.zip)
    ...
    When any --fix* flag is used and no fivem-scripts-backup-*.zip exists,
    ...
    # DEBUG
    --extract-debug [path]
                       Extract all modified files (.lua + .flao-bak) to a zip
                       for debugging (default: flao_debug_extract.zip)
    --split N          Split --extract-debug into multiple zips with N resources each
"""
...
# Backup file extension for FLAO
BACKUP_EXT = '.flao-bak'
...
def analyze_file_worker(args_tuple):
    ...
    resource_name, script_path, timeout, cache_threshold, experimental = args_tuple
    try:
        findings = analyze_file(script_path, cache_threshold=cache_threshold, experimental=experimental)
        return (resource_name, script_path, findings, None)
    except Exception as e:
        return (resource_name, script_path, [], str(e))
...
def backup_all_scripts(all_files, output_path=None, resources_root=None, quiet=False):
    """
    Backup all script files to a zip archive.

    Args:
        all_files: List of (resource_name, script_path) tuples
        output_path: Path to output zip file (auto-generated if None or 'auto')
        resources_root: Root resources directory (for relative paths in archive)
    ...
    if output_path is None or output_path == 'auto':
        ...
        filename = f'fivem-scripts-backup-{timestamp}.zip'
        # save in resources_root if provided, otherwise current directory
        if resources_root:
            output_path = Path(resources_root) / filename
        else:
    ...
    try:
        with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            for i, (resource_name, script_path) in enumerate(all_files):
                # create archive path preserving resource structure
                if resources_root and script_path.is_relative_to(resources_root):
                    archive_path = script_path.relative_to(resources_root)
                else:
                    # fallback: use resource_name/filename
                    archive_path = Path(resource_name) / script_path.name
...
def main():
    parser = argparse.ArgumentParser(
        description="FiveM Lua Auto Optimizer (FLAO)"
    )
    parser.add_argument(
        "path",
        nargs="?",
        help="Path to FiveM resources directory"
    )
    parser.add_argument(
        "--fix",
        action="store_true",
    ...
    parser.add_argument(
        "--experimental",
        action="store_true",
        help="Enable experimental features: string concat in loops + branch-aware call counting"
    )
    parser.add_argument(
        "--cache-threshold",
        type=int,
        default=4,
        metavar="N",
        help="Minimum function call count to trigger caching (default: 4, hot loops use N-1)"
    )
    parser.add_argument(
        "--backup",
        action="store_true",
        default=True,
        help=f"Create {BACKUP_EXT} files before modifying (default: True)"
    )
    parser.add_argument(
        "--no-backup",
        action="store_false",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Backup all scripts to a zip archive before any modifications"
    )
    parser.add_argument(
        "--report",
        type=str,
    ...
    parser.add_argument(
        "--revert",
        action="store_true",
        help=f"Restore all {BACKUP_EXT} backup files (undo FLAO fixes)"
    )
    parser.add_argument(
        "--list-backups",
        action="store_true",
        help=f"List all {BACKUP_EXT} backup files without restoring"
    )
    parser.add_argument(
        "--clean-backups",
        action="store_true",
        help=f"Remove all {BACKUP_EXT} backup files"
    )
    parser.add_argument(
        "--extract-debug",
        type=str,
        ...
        default=None,
        metavar="PATH",
        help=f"Extract all modified files (.lua + {BACKUP_EXT}) to a zip for debugging"
    )
    parser.add_argument(
        "--split",
        type=int,
        default=0,
        metavar="N",
        help="Split --extract-debug into multiple zips with N resources each (0 = no split)"
    )
    parser.add_argument(
        "--direct",
        action="store_true",
        help="Process path directly (single .lua file or folder with scripts, no resource structure)"
    )
    parser.add_argument(
        "--exclude",
        type=str,
        default=None,
        help="Path to file containing resource names to exclude (one per line)"
    )
    ...
    if args.path:
        ...
        resources_path = Path(clean_path)
    else:
        print("FiveM Lua Auto Optimizer (FLAO)")
        print("=" * 55)
        user_input = input("\nEnter path to FiveM resources directory: ").strip()
        if not user_input:
    ...
    if not resources_path.exists():
        print(f"Error: Path does not exist: {resources_path}")
        sys.exit(1)

    # discover resources and scripts
    print(f"\nScanning: {resources_path}")
    if args.direct:
        resources = discover_direct(resources_path)
    else:
        resources = discover_mods(resources_path)
                ...
                with open(exclude_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        if line and not line.startswith('#'):
                            excluded_resources.add(line)

                if excluded_resources:
                    before_count = len(resources)
                    resources = {name: scripts for name, scripts in resources.items() if name not in excluded_resources}
                    excluded_count = before_count - len(resources)
                    if excluded_count > 0:
                        print(f"Excluded {excluded_count} resources from {exclude_path.name}")
            except Exception as e:
    ...
    if not resources:
        if args.direct:
            print("No Lua scripts found in path.")
        else:
            print("No FiveM resources with scripts found.")
            print("Tip: Make sure the path contains resources with fxmanifest.lua or __resource.lua")
        sys.exit(0)

    total_scripts = sum(len(scripts) for scripts in resources.values())
    if args.direct:
        print(f"Found {total_scripts} Lua files (direct mode)\n")
    else:
        print(f"Found {len(resources)} resources with {total_scripts} Lua files\n")

    # handle backup operations
    if args.list_backups or args.revert or args.clean_backups:
        backup_files = []
        for resource_name, scripts in resources.items():
            for script_path in scripts:
                bak_path = script_path.with_suffix(script_path.suffix + BACKUP_EXT)
                if bak_path.exists():
                    backup_files.append((script_path, bak_path, resource_name))
        ...
        if args.list_backups:
            ...
            by_resource = {}
            for script_path, bak_path, resource_name in backup_files:
                if resource_name not in by_resource:
                    by_resource[resource_name] = []
                by_resource[resource_name].append(bak_path)

            for resource_name, baks in sorted(by_resource.items()):
    ...
    if args.extract_debug:
        ...
        for resource_name, scripts in resources.items():
            for script_path in scripts:
                bak_path = script_path.with_suffix(script_path.suffix + BACKUP_EXT)
                if bak_path.exists():
                    if resource_name not in files_by_resource:
                        files_by_resource[resource_name] = []
                    files_by_resource[resource_name].append((script_path, bak_path))

        if not files_by_resource:
            print(f"No modified files found (no {BACKUP_EXT} backups exist).")
            sys.exit(0)

        total_files = sum(len(files) for files in files_by_resource.values())
        total_resources = len(files_by_resource)
        ...
        resource_names = sorted(files_by_resource.keys())

        if args.split > 0:
            chunks = []
            for i in range(0, len(resource_names), args.split):
                chunks.append(resource_names[i:i + args.split])

            print(f"Extracting {total_files} modified files from {total_resources} resources into {len(chunks)} archives...")

            for chunk_idx, chunk_resources in enumerate(chunks, 1):
                chunk_zip = base_dir / f"{base_name}_{chunk_idx}.zip"
                chunk_file_count = 0

                with zipfile.ZipFile(chunk_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
                    for resource_name in chunk_resources:
                        for script_path, bak_path in files_by_resource[resource_name]:
                            arcname_script = f"{resource_name}/{script_path.name}"
                            arcname_bak = f"{resource_name}/{bak_path.name}"
                ...
                print(f"  [{chunk_idx}/{len(chunks)}] {chunk_zip.name}: {len(chunk_resources)} resources, {chunk_file_count} file pairs")
            ...
            print(f"\nExtracted {total_files} file pairs from {total_resources} resources:")
            for resource_name in resource_names:
                print(f"  [{resource_name}] {len(files_by_resource[resource_name])} files")
    ...
    for resource_name, scripts in resources.items():
        for script_path in scripts:
            all_files.append((resource_name, script_path))
    ...
    if fix_flags_set and not args.no_first_time_auto_backup and not args.backup_all_scripts:
        import glob
        backup_pattern = str(resources_path / "fivem-scripts-backup-*.zip")
        ...
        if not existing_backups:
            ...
            if backup_path is None:
                ...
                print("      3. Manually backup your resources folder first")
                return
            if not args.quiet:
                print()

    # backup all scripts if explicitly requested
    ...
    work_items = [
        (resource_name, script_path, args.timeout, args.cache_threshold, args.experimental)
        for resource_name, script_path in all_files
    ]
    ...
    try:
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    resource_name, script_path, findings, error = future.result()
                ...
                if error:
                    ...
                    if findings:
                        ...
                        for finding in findings:
                            reporter.add_finding(resource_name, script_path, finding)
    ...
    if pool_crashed:
        ...
        for resource_name, script_path in remaining:
            completed += 1
            if not args.quiet:
                progress = completed / len(all_files) * 100
                print(
    ...
    if args.fix or args.fix_debug or args.fix_yellow or args.experimental or args.fix_nil or args.remove_dead_code:
        ...
        # prepare work items, skip files that already have backup
        work_items = []
    ...
    # count distance_native findings
    distance_count = sum(1 for f in reporter.all_findings if f.pattern_name == 'distance_native')
    ...
    if distance_count > 0:
        findings_str += f", {distance_count} DISTANCE (use vector math)"
    ...
    if yellow_count > 0 and not args.fix_yellow:
        print("Tip: Run with --fix-yellow to also apply YELLOW fixes (review carefully)")
    ...
    if distance_count > 0:
        print("Tip: Replace GetDistanceBetweenCoords() with #(coords1 - coords2) for better performance")
    if (args.fix or args.fix_debug or args.fix_yellow or args.experimental or args.fix_nil or args.remove_dead_code):
        print(f"Tip: Run with --revert to undo all changes using {BACKUP_EXT} files")
